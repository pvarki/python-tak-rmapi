...
export function OnboardingHandler() {
    ...
    useEffect(() => {
        ...
        const storageKey = `${deploymentHash}-tak-onboarding-${callsign}-${userType}`;
        const roleChangeKey = `${deploymentHash}-tak-onboarding-role-${callsign}`;
        ...
        const completedSteps = localStorage.getItem(
            `${deploymentHash}-tak-onboarding-steps-${callsign}-${userType}`,
        );
    ...
    const handleComplete = () => {
        ...
        if (currentStep === relevantSteps.length - 1) {
            if (callsign && userType && deployment) {
                const deploymentHash = hashString(deployment);
                localStorage.setItem(
                    `${deploymentHash}-tak-onboarding-${callsign}-${userType}`,
                    "true",
                );
            }
            setOpen(false);
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div
                        className={cn(
                            ...
                            !imageError && "border border-border shadow-md",
