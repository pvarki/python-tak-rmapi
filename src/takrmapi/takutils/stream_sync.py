...
class MediaMTXTakSync:
    ...
    async def ensure_interop(self) -> None:
        ...
        async with self.rm_product_session() as session:
            resp = await session.post(f"/api/v1/product/interop/{config.MTX_PRODUCT_NAME}", json=build_interop_request())
    ...
    async def run_once(self) -> VideoSyncPlan:
        ...
        for payload in plan.update:
            ...
            if not update_resp["success"]:
                raise StreamSyncError(f"Unable to update TAK video connection '{payload['uuid']}': {update_resp['data']}")
