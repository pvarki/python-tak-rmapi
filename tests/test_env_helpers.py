...
from takrmapi.takutils import env_helpers
...
def test_tak_version_text(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Read the TAK version from a temporary image layout."""
    ...
    monkeypatch.setattr(env_helpers, "TAKCL_CORECONFIG_PATH", coreconfig_path)
    env_helpers.tak_version.cache_clear()
    try:
        assert env_helpers.tak_version() == (5, 8, 69)
    finally:
        env_helpers.tak_version.cache_clear()
