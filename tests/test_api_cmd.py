...
@pytest.mark.asyncio
@pytest.mark.parametrize("app_client", [{"test": "value", "xclientcert": False}], indirect=True)
async def test_get_cmd_run_something(app_client: TestClient) -> None:
    """
    /cmd/run-something
    at least "success" should come in response json
    """
    resp = await app_client.get("/api/v1/cmd/run-something")
    resp_dict: Dict[Any, Any] = resp.json()
    assert resp.status_code == 200
...
@pytest.mark.asyncio
@pytest.mark.parametrize("app_client", [{"test": "value", "xclientcert": False}], indirect=True)
async def test_get_cmd_run_something_cmd(app_client: TestClient) -> None:
    """
    /cmd/run-something-cmd
    at least "success" should come in response json
    """
    resp = await app_client.get("/api/v1/cmd/run-something-cmd")
    resp_dict: Dict[Any, Any] = resp.json()
    assert resp.status_code == 200
    assert resp_dict["success"] is not None
