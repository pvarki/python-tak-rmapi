...
@pytest.mark.asyncio
@pytest.mark.parametrize("app_client", [{"test": "value", "xclientcert": False}], indirect=True)
async def test_get_release_download_text(app_client: TestClient) -> None:
    """
    /config/active-config
    at least "success" should come in response json
    """
    resp = await app_client.get("/api/v1/release/download-text")
    resp_dict: Dict[Any, Any] = resp.json()
    assert resp.status_code == 200
...
@pytest.mark.asyncio
@pytest.mark.parametrize("app_client", [{"test": "value", "xclientcert": False}], indirect=True)
async def test_get_release_download_package(app_client: TestClient) -> None:
    """
    /config/active-config
    at least "success" should come in response json
    """
    resp = await app_client.get("/api/v1/release/download-package")
    resp_dict: Dict[Any, Any] = resp.json()
    assert resp.status_code == 200
    assert resp_dict["success"] is not None
