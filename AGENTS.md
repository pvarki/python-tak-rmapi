...
certificate, registers them in TAK Server via JNI, and manages their access.
...
**User management via JNI:** User registration, admin role changes, and deletion use
the Java Native Interface (JNI) to call TAK Server's Java APIs.

Each user gets two cert registrations: `<callsign>` and `<callsign>_rm` (the latter for
the RASENMAEHER mTLS client cert). Both are managed via JNI.
...
1. **User management requires JNI.** User registration, admin role changes, and deletion
   depend on the JNI integration with TAK Server's Java APIs. Account for this dependency
   when debugging or testing lifecycle callbacks.
